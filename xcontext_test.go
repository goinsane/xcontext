...
func TestDelayContext(t *testing.T) {
	...
	if period < timeout || period < delay {
...
func TestDelayAfterContext(t *testing.T) {
	var timeout, delay, cancel, period time.Duration

	timeout, delay, cancel = 1*time.Second, 2*time.Second, 0
	t.Logf("test with timeout=%v delay=%v cancel=%v", timeout, delay, cancel)
	period = testDelayAfterContext(timeout, delay, cancel)
	if period < timeout || period < delay {
		t.Errorf("test failed with timeout=%v delay=%v cancel=%v period=%v", timeout, delay, cancel, period)
	} else {
		t.Logf("test ok with timeout=%v delay=%v cancel=%v period=%v", timeout, delay, cancel, period)
	...
	t.Logf("test with timeout=%v delay=%v cancel=%v", timeout, delay, cancel)
	period = testDelayAfterContext(timeout, delay, cancel)
	if period >= timeout+delay || period < cancel {
		t.Errorf("test failed with timeout=%v delay=%v cancel=%v period=%v", timeout, delay, cancel, period)
	} else {
		t.Logf("test ok with timeout=%v delay=%v cancel=%v period=%v", timeout, delay, cancel, period)
	}
...
func testDelayAfterContext(timeout, delay, cancel time.Duration) time.Duration {
	startTm := time.Now()
	ctx, ctxCancel := context.WithTimeout(context.Background(), timeout)
	defer ctxCancel()
	newCtx, newCtxCancel := DelayAfterContext(ctx, delay)
	defer newCtxCancel()
	if cancel > 0 {
		go func() {
			<-time.After(cancel)
